...
var refreshToken = (req, res) => {
  ...
  try {
    jwt2.verify(token, ENV_VARS.REFRESH_SECRET, async (err, decoded) => {
      if (err) {
        res.status(403).json({
          success: false,
...
var Comment = model3("Comment", commentSchema);

// src/configs/gemini.ts
import { GoogleGenAI } from "@google/genai";
var ai = new GoogleGenAI({ apiKey: ENV_VARS.GEMINI_API_KEY });
...
var authenticateJWT = (req, res, next) => {
  passport2.authenticate("jwt", { session: false }, (err, user) => {
    if (err) {
      res.status(500).json({
        success: false,
...
var createBlogSchema = z2.object({
  title: z2.string({
  ...
  category: z2.enum(blogCategories, {
    required_error: "Category is required",
    message: `Category must be one of these: ${blogCategories.join(" | ")}`
  }),
  isPublished: z2.union([
    z2.boolean(),
    z2.literal("true").transform(() => true),
    z2.literal("false").transform(() => false)
  ]).catch(false).default(false)
});
var generateDescriptionSchema = z2.object({
  prompt: z2.string({ required_error: "Prompt text is required" }).trim().min(10, "Prompt must be at least 10 characters")
});
...
var newsletterSubscriberSchema = new Schema4({
  email: {
    type: String,
...
var NewsletterSubscriber = model4("NewsletterSubscriber", newsletterSubscriberSchema);
